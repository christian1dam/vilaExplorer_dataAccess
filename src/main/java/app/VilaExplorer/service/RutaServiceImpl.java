...
RutaServiceImpl implements RutaService {
    ...
    // NUEVO MÉTODO para actualizar rutas que no son predefinidas
    @Override
    public Ruta updateRutaNoPredefinida(Long id, Ruta rutaDetails) throws RutaNotFoundException {
        // 1. Verificar si la ruta existe (usando el nombre de la propiedad correcto)
        Ruta rutaFromDB = rutaRepository.findByIdRutaAndPredefinidaFalse(id)
                .orElseThrow(() -> new RutaNotFoundException("No se encontró la ruta (O es predefinida), con ID: " + id));
